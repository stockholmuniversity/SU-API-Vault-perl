...
sub login {
    ...
    if( $timeout > ($self->{exp_time}) ) {
        ...
        my %post_data = ( "role_id" => $self->{role_id}, "secret_id" => $self->{secret_id});
...
sub get_secret {
    ...
    my $timeout = $time + 30;
    my ($self, $path) = @_;
    if( ! $self->{last_secret} or  ($self->{last_secret}->{path} ne $path) or ($self->{last_secret}->{access_time} < ($time - 300))) {
